...
// Unit 1: serial flash
static constexpr long MaxSpiClock1 = 50'000'000;
...
enum RA8876_InitValues
{
	...
	SFL_CTRL_Init0 = SFL_CTRL_Select0 | SFL_CTRL_ReadCommand0B | 
		SFL_CTRL_AddrBits24 | SFL_CTRL_ModeFont,

	// SPI Serial flash at CS1
	SFL_CTRL_Init1 = SFL_CTRL_Select1 | SFL_CTRL_ReadCommandBB | 
		SFL_CTRL_AddrBits24 | SFL_CTRL_ModeDma,
...
void RA8876::ExternalFont(uint uCharHeight, uint uFont)
{
	WriteReg(CCR0, uCharHeight | CCR0_CharSourceExternal);
	WriteReg(GTFNT_CR, uFont);
	WriteReg(SFL_CTRL, SFL_CTRL_Init0);
	WriteReg(SPI_DIVSOR, SpiDivisor0);
}
